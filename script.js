...
function printproduct() {
    ...
    let getrec = JSON.parse(localStorage.getItem('product')) || [];
    let print = document.querySelector('#row');

    let str = '';
    getrec.forEach((rec, id) => {
        str +=
            `<div class="col-md-4 col-lg-3 col-12">
  <div class="card h-100 shadow-lg p-3 card-hover border-2 rounded-5 border-black">
    
    <div class="card-body d-flex flex-column justify-content-center p-0 text-black" onclick="return carddetail(${rec.id})">
      <img src="${rec.thumbnail}" class="card-img-top" alt="Product Image">
      <h5 class="card-title text-black">${rec.title}</h5>
      <p class="card-text small text-black mb-2">${rec.description}</p>

      <div class="d-flex justify-content-between align-items-center mb-2">
        <span class="fw-bold fs-5 text-black">₹${rec.price}</span>
      </div>
    </div>

    <div class="cart-buttons d-flex align-items-center justify-content-center">
      <button class="btn btn-dark btn-sm p-2 me-2 rounded-pill" onclick="return additem(${rec.id})">Add to Cart</button>
      <button class="btn btn-outline-dark p-2 rounded-pill">Buy Now</button>
    </div>
  </div>
</div>`
    });
...
function carddetail(id) {
  ...
  <div class="row border border-black rounded-4 pb-3">
    <div class="col-12 d-flex justify-content-end p-2">
      <button class="btn btn-danger badge" onclick="backtolist()">X</button>
    </div>

    <!-- Row for product image and details -->
    <div class="row g-3">
      <div class="col-md-5 col-xl-4 col-12 d-flex justify-content-center align-items-center">
        <img src="${product.thumbnail}" class="rounded shadow-lg w-75" alt="${product.title}">
      </div>
      ...
      <div class="col-md-7 col-xl-7 col-12">
        ...
        <p><strong>Status:</strong> ${product.availabilityStatus || "N/A"}</p>
        <p><strong>Return-policy:</strong> ${product.returnPolicy}</p>

        <div class="d-flex gap-2 mt-3">
          <button class="btn btn-dark btn-lg" onclick="additem(${product.id})">Add to Cart</button>
